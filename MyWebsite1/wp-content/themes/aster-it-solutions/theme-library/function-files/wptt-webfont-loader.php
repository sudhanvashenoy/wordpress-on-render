...
if ( ! class_exists( 'WPTT_WebFont_Loader' ) ) {
	...
	class WPTT_WebFont_Loader {
		 ...
		 * @access public
		 * @since 1.1.0
		 * @param string $aster_it_solutions_url The remote URL.
		 */
		public function __construct( $aster_it_solutions_url = '' ) {
			$this->remote_url = $aster_it_solutions_url;
		...
		public function get_local_files_from_css() {
			...
			foreach ( $font_files as $font_family => $files ) {
				...
				foreach ( $files as $aster_it_solutions_url ) {

					// Get the filename.
					$filename  = basename( wp_parse_url( $aster_it_solutions_url, PHP_URL_PATH ) );
					...
					$tmp_path = download_url( $aster_it_solutions_url );
			...
			if ( $change ) {
				...
				foreach ( $stored as $aster_it_solutions_url => $path ) {
					if ( ! file_exists( $path ) ) {
						unset( $stored[ $aster_it_solutions_url ] );
					}
				}
		 ...
		 * @access public
		 * @since 1.0.0
		 * @param string $aster_it_solutions_format The format to be used. Use "woff" or "woff2".
		 * @return void
		 */
		public function set_font_format( $aster_it_solutions_format = 'woff2' ) {
			$this->font_format = $aster_it_solutions_format;
		}
		 ...
		 * @access protected
		 *
		 * @param string $aster_it_solutions_url The URL.
		 *
		 * @return string
		 */
		protected function get_absolute_path( $aster_it_solutions_url ) {

			// If dealing with a root-relative URL.
			if ( 0 === stripos( $aster_it_solutions_url, '/' ) ) {
				$aster_it_solutions_parsed_url = parse_url( $this->remote_url );
				return $aster_it_solutions_parsed_url['scheme'] . '://' . $aster_it_solutions_parsed_url['hostname'] . $aster_it_solutions_url;
			}

			return $aster_it_solutions_url;
		}
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_styles' ) ) {
	 ...
	 * @since 1.0.0
	 *
	 * @param string $aster_it_solutions_url    The URL of the remote webfont.
	 * @param string $aster_it_solutions_format The font-format. If you need to support IE, change this to "woff".
	 *
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_styles( $aster_it_solutions_url, $aster_it_solutions_format = 'woff2' ) {
		$aster_it_solutions_font = new WPTT_WebFont_Loader( $aster_it_solutions_url );
		$aster_it_solutions_font->set_font_format( $aster_it_solutions_format );
		return $aster_it_solutions_font->get_styles();
	}
}
...
if ( ! function_exists( 'wptt_get_webfont_url' ) ) {
	 ...
	 * @return string Returns the CSS.
	 */
	function wptt_get_webfont_url( $aster_it_solutions_url, $aster_it_solutions_format = 'woff2' ) {
		$aster_it_solutions_font = new WPTT_WebFont_Loader( $aster_it_solutions_url );
		$aster_it_solutions_font->set_font_format( $aster_it_solutions_format );
		return $aster_it_solutions_font->get_url();
	}
}